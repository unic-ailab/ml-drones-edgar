...
from flockai.models.sensors.velocity import VelocitySensor
from flockai.models.sensors.lin_acceleration import LinearAccelerationSensor
...
class wind_speed_lr(FlockAIClassifier):
    """
    ...
    def _load_model(self):
        ...
        self.velocity_sensor=VelocitySensor('data/velocity_y_data.txt')
        self.lin_acceleration_sensor=LinearAccelerationSensor('data/linear_acceleration_y_data.txt')   

    def _get_model_input(self):
       velocity_avg = self.velocity_sensor.get_values()
       lin_acceleration_avg = self.lin_acceleration_sensor.get_values()
       return velocity_avg, lin_acceleration_avg

    def predict(self):
        if controller.getTime() % self.periodicity != 0.0:  # get access to controller functions
            return None

        velocity_data, lin_acceleration_data = self._get_model_input() 
        input_vector = np.array([velocity_data, lin_acceleration_data]).reshape(1, -1)
...
"""""""""""""""""""""""""""""""""""""""""""""
controller.model = wind_speed_lr()
controller.run()
